"""
Run all dataset × model combinations and produce a unified comparison.

Train/test split: stratified 80/20 hold-out (random_state=42).
Cross-validation: 5-fold on the TRAINING set only — test set is never seen during training.

Models:
  tfidf    — TF-IDF (1–2 grams) + Logistic Regression
  semantic — Sentence-Transformer embeddings (all-MiniLM-L6-v2) + Logistic Regression

Datasets: v1 (897), v2 (897 relabeled), v2_mega (1650)

Outputs:
  outputs/metrics/summary.csv           — all combos in one table
  outputs/metrics/<tag>_report.csv      — per-class metrics per combo
  outputs/plots/comparison.png          — side-by-side CV vs test F1
  outputs/plots/confusion_<tag>.png     — confusion matrices per combo
  outputs/plots/keywords_<tag>_<cls>.png
  outputs/keywords/<tag>_<cls>.csv
"""
...
from src.config import DATASETS, CLASSES, SEMANTIC_MODEL, OUT_METRICS, OUT_PLOTS, OUT_KEYWORDS
...
summary_rows = []
confusion_results = {}   # tag → result dict (for confusion matrix plot)
...
print("\nSaved: outputs/plots/comparison.png")
